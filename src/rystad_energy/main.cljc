(ns rystad_energy.main
  "Kotodama WASM entrypoint for the Rystad_energy clean-room actor (L4) — Clojure port.
  ...
  facts (`rystad_energy.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "rystad_energy")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"rystad_energy.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rystad_energy-compat" :tier tier :entities entities} 200])
...
(defn start! [] :rystad_energy-compat/ready)
